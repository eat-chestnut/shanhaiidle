                ...
                ->tabs([
                    Tab::make('基础信息')
                        ->schema([
                            Section::make('基础字段')
                                ->schema([
                                    TextInput::make('id')->label('怪物 ID')->required()->maxLength(64)->unique(ignoreRecord: true)->disabled(fn (?Monster $record): bool => $record !== null),
                                    TextInput::make('name')->label('名称')->required()->maxLength(255),
                                    Select::make('kind')->label('类型')->required()->options(static::kindOptions())->rule(new In(['normal', 'elite', 'boss']))->helperText('怪物类型会影响掉落与图鉴展示。'),
                                    FileUpload::make('icon')->label('图标')->disk('public')->directory('config/monster-icons')->image()->imagePreviewHeight('120'),
