...
class BattleStartEffectApplier
{
    /**
    ...
    public function apply(array $unitRuntimeState, array $runtimeEffects): array
    {
        ...
        $unitRuntimeState['shield'] = max(0, $this->normalizeNumeric($unitRuntimeState['shield'] ?? 0));

        $allowedEffectIds = $this->buildEffectIdentityMap($runtimeEffects);
        $appliedEffects = [];

        foreach ($unitRuntimeState['runtime_effects'] as $effectIndex => $runtimeEffect) {
            ...
            if (trim((string) ($runtimeEffect['trigger_timing'] ?? '')) !== 'on_battle_start') {
                continue;
            }

            $effectType = trim((string) ($runtimeEffect['effect_type'] ?? ''));
            if ($effectType === 'battle_start_shield') {
                $unitRuntimeState['shield'] = max(0, $this->sumNumeric(
                    $unitRuntimeState['shield'],
                    $runtimeEffect['value'] ?? 0
                ));
                $unitRuntimeState['runtime_effects'][$effectIndex]['enabled'] = false;
                $appliedEffects[] = $runtimeEffect;

                continue;
            }

            if ($effectType === 'battle_start_modifier') {
